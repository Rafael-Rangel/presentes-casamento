    ...
    >
      ...
      {!profileComplete ? (
        <p className="text-sm text-muted">
          Para a primeira reserva, precisamos do teu nome, telefone e de como conheces os
          noivos. Também podes completar estes dados em{" "}
          <a href="/conta/dados" className="font-medium text-ocean underline">
            Os teus dados
          </a>
          .
        </p>
      ...
      {!profileComplete ? (
        <div className="space-y-4 rounded-xl border border-ocean/15 bg-ocean/5 p-3 sm:p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-ocean-deep">
            Os teus dados
          </p>
          ...
          <label className="block text-sm">
            ...
            <textarea
              ...
              className="mt-2 min-h-[4rem] w-full rounded-xl border border-border bg-paper px-3 py-2.5 text-base text-ink shadow-inner outline-none focus:ring-2 focus:ring-ocean/25 sm:text-sm"
            />
          </label>
          <label className="flex min-h-[44px] cursor-pointer items-start gap-3 text-sm text-muted">
            <input
              type="checkbox"
              name="marketingOptInProfile"
              defaultChecked={defaultMarketingOptIn}
              className="mt-1 size-5 shrink-0 rounded border-border"
            />
            <span>Lembretes ocasionais sobre a lista (opcional).</span>
          </label>
        </div>
      ) : null}
      {state.error ? (
      ...
      {state.ok ? (
        <p className="text-sm font-medium text-ocean">Reserva criada. A redirecionar…</p>
      ) : null}
      <label className="block text-sm">
