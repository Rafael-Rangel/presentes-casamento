-- Baseline inicial: alinhado com supabase/schema.sql (CLI: supabase db push).
-- Projeto já criado À MÃO no SQL Editor? Não faças push desta migração em cima —
-- usa apenas seed ou repair; caso contrário podes ver erros “already exists”.
-- Depois: promover utilizador admin no Dashboard se precisares de políticas por role.
